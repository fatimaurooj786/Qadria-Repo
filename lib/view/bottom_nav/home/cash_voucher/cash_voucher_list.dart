...
import 'package:qadria/view/bottom_nav/home/cash_voucher/recieve_screen.dart';
import 'package:qadria/view_models/lists/cash_voucher_list_controller.dart';

import 'expense_screen.dart';  // Import the Expense Screen
import 'pay_screen.dart';      // Import the Pay Screen
//import 'receive_screen.dart';  // Import the Receive Screen

class CashVoucherList extends StatefulWidget {
  const CashVoucherList({super.key});

  @override
  State<CashVoucherList> createState() => _CashVoucherListState();
}

class _CashVoucherListState extends State<CashVoucherList> {
  final controller = Get.put(CashVoucherListController());
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.pixels == _scrollController.position.maxScrollExtent) {
        if (selectedPartyType == '' && selectedParty == '' && selectedDate == '') {
          controller.fetchCashVouchers(); // Load more items if filters are not applied
        }
      }
    });
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Obx(() {
            if (controller.isLoading.value && controller.cashVoucherList.isEmpty) {
              return const Center(child: CircularProgressIndicator());
            } else {
              if (controller.cashVoucherList.isEmpty) {
                return const Expanded(
                  child: Center(child: Text('No cash vouchers available', style: TextStyle(fontSize: 18))),
                );
              }
              return Expanded(
                child: ListView.builder(
                  controller: _scrollController,
                  itemCount: controller.cashVoucherList.length + (controller.isLoading.value ? 1 : 0),
                  itemBuilder: (context, index) {
                    if (index == controller.cashVoucherList.length) {
                      return const Center(child: CircularProgressIndicator());
                    }
                    final invoice = controller.cashVoucherList[index];
